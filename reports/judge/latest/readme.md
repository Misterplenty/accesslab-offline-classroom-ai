...
Instead of sending student questions and classroom data to the cloud, AccessLab runs entirely on local hardware. It grounds its answers securely in teacher-uploaded materials, helps beginners repair Python code, and gracefully abstains when evidence is weak.
...
Experience AccessLab directly in your browser. (Note: The live demo runs on a constrained free-tier CPU, so generation may take a few moments).
...
- **Honest Abstention:** If the retrieved evidence is weak or empty, AccessLab explicitly shows uncertainty rather than hallucinating an answer.
